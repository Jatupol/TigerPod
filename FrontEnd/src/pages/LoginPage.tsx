...
const LoginPage: React.FC = () => {
  ...
  const triggerAutoImport = async (): Promise<void> => {
    try {
      console.log('🔄 Checking if auto-import should run...');
      ...
      if (shouldImportCheckinResult.success && shouldImportCheckinResult.data?.shouldImport) {
        console.log('🔄 Starting auto-import for inf-checkin...');

        const checkinResult = await api.post<{ imported: number; message?: string }>('/inf-checkin/import/auto');

        if (checkinResult.success) {
          console.log(`✅ Checkin auto-import completed: ${checkinResult.data?.imported} records imported`);
        } else {
          console.log(`⚠️ Checkin auto-import warning: ${checkinResult.message}`);
        }
      } else if (shouldImportCheckinResult.success) {
        console.log(`⏳ Checkin import not needed yet: ${shouldImportCheckinResult.data?.message}`);
      } else {
        console.log(`⚠️ Cannot check checkin import status: ${shouldImportCheckinResult.message}`);
      }
      ...
      if (shouldImportLotinputResult.success && shouldImportLotinputResult.data?.shouldImport) {
        console.log('🔄 Starting auto-import for inf-lotinput...');

        const lotinputResult = await api.post<{ imported: number; message?: string }>('/inf-lotinput/import', {});

        if (lotinputResult.success) {
          console.log(`✅ Lotinput auto-import completed: ${lotinputResult.data?.imported} records imported`);
        } else {
          console.log(`⚠️ Lotinput auto-import warning: ${lotinputResult.message}`);
        }
      } else if (shouldImportLotinputResult.success) {
        console.log(`⏳ Lotinput import not needed yet: ${shouldImportLotinputResult.data?.message}`);
      } else {
        console.log(`⚠️ Cannot check lotinput import status: ${shouldImportLotinputResult.message}`);
      }
