...
const InspectionStationPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState<FilterOptions>({
    dateStart: inspectionStationService.getSevenDaysAgoDate(),
  ...
  const loadInspectionData = async () => {
    try {
      ...
      const result = await inspectionDataService.getAll({
        station: config.code,
        page: 1,
  ...
  const stats = useMemo(() => {
    ...
    const aqlWarnings = filteredRecords.filter(r =>
      !isValidAQLSampling(r.fviLotQty ?? 0, r.generalSamplingQty ?? 0)
    ).length;

    return { total, passed, rejected, pending, passRate, aqlWarnings };
  }, [filteredRecords]);
    ...
    navigate('/inspection/defect/new', {
      state: {
        inspectionData: {
          ...
          station: config.code // Add station code (OQA, OBA, or SIV)
        }
      }
    });
  };
  ...
  const handleGenerateFVI = async (record: InspectionRecord) => {
    try {
      ...
      if (!inspectionStationService.canCreateSIV(record, config.code)) {
        sound.playWarning();
        if (config.code !== 'OQA') {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${theme.background} p-6`}>
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className={`bg-gradient-to-r ${theme.header} p-6`}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                ...
                {stats.aqlWarnings > 0 && (
                  <div className="bg-orange-500/20 backdrop-blur-sm rounded-xl px-4 py-3 border border-orange-400/30 text-center min-w-[100px]">
                    <div className="text-orange-100 font-bold text-xl">{formatNumber(stats.aqlWarnings.toString())}</div>
                    <div className="text-white/70 text-xs">⚠ AQL Warnings</div>
                  </div>
                )}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4 text-xs text-gray-600">
                ...
                <span className="flex items-center text-red-600">
                  <span className="px-2 py-0.5 rounded text-xs font-medium bg-red-100 border border-red-200 mr-1">⚠ AQL</span> Below Minimum Sampling
                </span>
              </div>
            </div>
          </div>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {!isValidAQLSampling(record.fviLotQty ?? 0, record.generalSamplingQty ?? 0) && (
                            <span
                              className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 border border-red-200"
                              title={`AQL Warning: FVI Lot ${getAQLRangeDescription(record.fviLotQty ?? 0)} requires minimum ${getMinimumGeneralSamplingQty(record.fviLotQty ?? 0)} samples`}
                            >
                              ⚠ AQL
                            </span>
                          )}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {config.code === 'OQA' && record.judgment === 'Pass' && (
                            <button
        ...
        {/* AQL Warnings Panel - Red Background like Configuration Warnings */}
        {stats.aqlWarnings > 0 && (
            ...
            <div className="flex items-start space-x-2">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-bold text-red-800 mb-3">
                  AQL Sampling Warnings ({stats.aqlWarnings} {stats.aqlWarnings === 1 ? 'record' : 'records'})
                </h4>
                <p className="text-sm text-red-700 mb-3">
                  The following records have General Sampling Quantity below the minimum AQL requirements:
                </p>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {filteredRecords
                    .filter(r => !isValidAQLSampling(r.fviLotQty ?? 0, r.generalSamplingQty ?? 0))
                    .map((record, index) => (
                      <div key={record.id} className="bg-white border border-red-200 rounded p-3 text-sm">
                        <div className="flex justify-between items-start">
                          <div className="space-y-1">
                            <div className="font-medium text-red-900">
                              {index + 1}. Sampling No: <span className="font-bold">{record.inspectionNo}</span>
                            </div>
                            <div className="text-red-700">
                              Lot No: {record.lotno} | Item: {record.itemno} | Model: {record.model}
                            </div>
                            <div className="text-red-600 text-xs mt-1 flex items-center space-x-4">
                              <span>FVI Lot Qty: <strong>{formatNumber((record.fviLotQty ?? 0).toString())}</strong></span>
                              <span>General Sampling: <strong className="text-red-700">{formatNumber((record.generalSamplingQty ?? 0).toString())}</strong></span>
                              <span className="text-red-800 font-semibold">
                                ⚠ Required: {getMinimumGeneralSamplingQty(record.fviLotQty ?? 0)}
                              </span>
                            </div>
                            <div className="text-xs text-red-500 mt-1">
                              AQL Range: {getAQLRangeDescription(record.fviLotQty ?? 0)} units
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="mt-3 p-3 bg-red-100 rounded text-xs text-red-800">
                  ...
                  Please verify sampling quantities meet the minimum requirements for the FVI lot size.
                </div>
              </div>
            </div>
          </div>
