// server/src/entities/reports/model.ts
// ===== LAR REPORT MODEL =====
// Line Acceptance Rate (LAR) Report Entity Model
// Manufacturing Quality Control System - Report Data Access Layer
...
export class ReportModel {
  ...
  // ==================== MAIN REPORT QUERY ====================
  /**
   * Get LAR chart data with the provided SQL query
   * Query returns: ww, total_lot, total_pass_lot, total_fail_lot, total_inspection,
   *                  total_ng, lar, dppm
   */
  async getLARChart(params: LARReportQueryParams = {}): Promise<LARReportRecord[]> {
    try {
      const {
        model,
        yearFrom,
      ...
      if (yearFrom && wwFrom && yearTo && wwTo) {
        values.push(`${yearFrom}${wwFrom.toString()}`);//$1
        values.push(`${yearTo}${wwTo.toString()}`);//$2
      }
      ...
      // Main LAR report query - exactly as provided
      const query = `
  ...
  /**
   * Get LAR defect data with the provided SQL query
   * Query returns: ww, defectname, total_ng
   */
  async getLARDefect(params: LARReportQueryParams = {}): Promise<LARReportRecord[]> {
    try {
      const {
        model,
        yearFrom,
   ...
   /**
   * Get Seagate IQA Result  data with the provided SQL query
   * Query returns:     fy, ww,fyww, model, total_inspection_lot, acceptable_lot, rejected_lot, rejected_qty, lar
   */
  async getSeagateIQAResult(params: SeagateIQAResultQueryParams): Promise<SeagateIQAResultReportRecord[]> {
    try {
      const {
        year,
        ww
  ...
  /**
   * Get SGT IQA Trend chart data with the provided SQL query
   * Query returns: ww, total_lot, total_pass_lot, total_fail_lot, total_inspection,
   *                  total_ng, lar, dppm
   */
  async getSGTIQATrendChart(params: SGTIQATrendQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      const {
        model,
        product_type,
      ...
      const conditions: string[] = [`P.partno = Q.item`];
  ...
  /**
   * Get SGT IQA Trend defect data with the provided SQL query
   * Query returns: ww, defectname, total_ng
   */
  async getSGTIQATrendDefect(params:  SGTIQATrendQueryParams = {}): Promise<SGTIQATrendRecord[]> {
    try {
      const {
        model,
        product_type,
  ...
  /**
   * Get OQA DPPM OVerall chart data with the provided SQL query
   * Query returns: ww, total_lot, total_pass_lot, total_fail_lot, total_inspection,
   *                  total_ng, lar, dppm
   */
  async getOQADppmOverallChart(params: OQADPPMOverallQueryParams = {}): Promise<OQADPPMOverallRecord[]> {
    try {
      const {
        year,
        ww
  ...
  /**
   * Get OQA DPPM OVerall defect data with the provided SQL query
   * Query returns: ww, defectname, total_ng
   */
  async getOQADppmOverallDefect(params:  OQADPPMOverallQueryParams = {}): Promise<OQADPPMOverallRecord[]> {
    try {
      const {
        year,
        ww
  ...
  /**
   * Get available models from inspectiondata
   * Returns combined model and version strings
   */
  ...
  /**
   * Get available fiscal years from inspectiondata
   * Returns distinct fiscal years ordered by fy
   */
  ...
  /**
   * Get available work weeks for a specific fiscal year from inspectiondata
   * Returns distinct work weeks ordered by ww, shown as 2-digit format
   * @param fiscalYear - The fiscal year to filter by (optional)
   */
  async getWorkWeeks(fiscalYear?: string): Promise<string[]> {
    try {
...
/**
 * Factory function to create a Report model instance
 */
export function createLARReportModel(db: Pool): ReportModel {
...
✅ Exact SQL query as provided by user
✅ LAR calculation: (TOTAL_PASS_LOT/TOTAL_LOT) * 100.0
✅ DPPM calculation: (Total_NG/Total_Inspection) * 1000000.0
✅ LEFT JOIN for defect data aggregation
✅ Grouped by work week (ww) and defect name

FLEXIBLE FILTERING:
✅ Work week range filtering (wwFrom, wwTo)
✅ Date range filtering (dateFrom, dateTo)
✅ Station filtering (default: 'OQA')
✅ Round filtering (default: 1)
✅ Parameterized queries for SQL injection prevention

ADDITIONAL FEATURES:
✅ Statistics aggregation method
✅ Top defects ranking
✅ Average LAR and DPPM calculation
...
✅ Connection pooling through PostgreSQL pool
✅ Transaction safety
*/
